...
交互器：负责与用户进行信息交互，包括读入指令和输出从账户系统、图书系统或日志系统得到的返回值。

语法检查器：检查从交互器处得到的指令是否合法，如果不合法直接返回，如果合法根据指令类型将指令传递给账户系统、图书系统或日志系统。

日志系统：负责管理日志。具体地说，支持：
- 以 BlockList 形式维护一个财务日志 finance.data, 信息形式为 $\texttt{pair<Id,tuple<IncomeSum,ExpenseSum>>}$ （即收入与支出的前缀和）；以 BlockList 形式维护一个总体日志 log.data，信息形式为 $\texttt{pair<Id,Info>}$。

账户系统：负责管理账户。具体地说，支持：

- 以 BlockList 形式维护一个登录栈 stack.data 和一个账户文件 account.data，信息形式为 $\texttt{pair<UserId,tuple<Password,Privilege,Username,Selected>>}$，其中 Selected 使用 $\texttt{pair<ISBN,tuple<...>>}$。
...
- 修改图书信息。如果未选中图书或附加参数重复或附加参数内容为空或将 ISBN 改为原来的 ISBN 或 Keyword 包含重复信息段则 fail，否则修改四个文件和 stack.data。可以 0 元卖。
- 图书进货。如果未选中图书或购入数量为非正整数或交易总额为非正数着 fail，否则修改四个文件和 stack.data。不能 0 元买。
